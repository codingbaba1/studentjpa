...
public class StudentJpaApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Student s = Student.builder().name("imran").address("delhi").build();
		Student s1 = Student.builder().name("sonali").address("chennai").build();
		Student s2 = Student.builder().name("shobha").address("kolkata").build();
		Student s3 = Student.builder().name("neha").address("bangalore").build();
